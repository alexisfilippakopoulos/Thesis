...
class XBertLayer(nn.Module):
    def __init__(self, config, add_adapter=True):
        ...
        if add_adapter==True:
            ...
            # ---------------------cross attention----------------------#
            self.adapter_conv_audio = nn.Conv1d(in_channels=config.audio_dim, out_channels=config.hidden_size, kernel_size=config.kernel_size)
            self.adapter_conv_vision = nn.Conv1d(in_channels=config.vision_dim, out_channels=config.hidden_size, kernel_size=config.kernel_size)
    ...
    def forward(
        ...
        if self.add_adapter == True:
            ...
            router_in = torch.stack(
                (attention_output, audio_t + attention_output, vision_t + attention_output),
                dim=2,  # (B, L, 3, D)
            )
            router_in = router_in.view(-1, 3, hidden_dim)  # (T, 3, D)

            # --- Gating ---
            gate_logits = self.adapter_atten_gate(router_in).view(T, -1)  # (T, num_experts)
            gate_p = F.softmax(gate_logits, dim=1, dtype=torch.float).to(attention_output.dtype)
            weights, selected_experts = torch.topk(gate_logits, self.TopK, dim=1)  # (T, K)
            weights = F.softmax(weights, dim=1, dtype=torch.float).to(attention_output.dtype)

            # --- Experts (full-sequence forward) ---
            expert_outputs = [
                self.adapter_1(attention_output),
                self.adapter_2(attention_output),
                self.adapter_audio_1(audio_t + attention_output),
                self.adapter_audio_2(audio_t + attention_output),
                self.adapter_vision_1(vision_t + attention_output),
                self.adapter_vision_2(vision_t + attention_output),
            ]
            expert_outputs = torch.stack(expert_outputs, dim=0)              # (num_experts, B, L, D)
            expert_outputs = expert_outputs.permute(1, 2, 0, 3).reshape(T, self.num_experts, hidden_dim)
            ...
            weighted_sum = (weights_expanded * selected_outputs).sum(dim=1)  # (T, D)
            results = weighted_sum.view(batch_size, sequence_length, hidden_dim)
            results = (32 / self.rank) * results

            # --- Load balancing loss (unchanged) ---
            f = torch.zeros(self.num_experts, device=attention_output.device)
            P = torch.zeros(self.num_experts, device=attention_output.device)
...
class XBertModel(BertPreTrainedModel):
    """
